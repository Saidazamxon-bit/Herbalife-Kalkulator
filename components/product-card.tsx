...
export function ProductCard({
  ...
  useEffect(() => {
    ...
    setUnlikes(product.unlikes || 0);
    setMounted(true);
  }, [product.likes, product.unlikes]);
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-slate-900 rounded-xl shadow-md hover:shadow-lg transition-all duration-300 overflow-hidden hover:-translate-y-0.5 border border-green-100 dark:border-green-900">
      ...
      <div className="flex flex-col flex-grow p-3 sm:p-4">
        {/* Product Name */}
        <h3 className="text-sm sm:text-base font-semibold text-slate-900 dark:text-white line-clamp-2 mb-2 tracking-tight font-display">
          {product.name}
        </h3>
        ...
        <div className="flex gap-1 pt-2 border-t border-slate-200 dark:border-slate-700">
          ...
          >
            ...
            <span suppressHydrationWarning>{mounted ? likes : 0}</span>
          </button>
          <button
          ...
          >
            ...
            <span suppressHydrationWarning>{mounted ? unlikes : 0}</span>
          </button>
        </div>
      </div>
    </div>
